...
function ProjectLinks({ project }: { project: ProjectItem }) {
    return (
        <div style={{
            display: 'flex', gap: '12px', paddingTop: '8px', flexWrap: 'wrap',
            ...
            {project.github && (
                <a
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: COLORS.textMuted, fontWeight: 500 }}
                >
            ...
            {project.appStore && (
                <a
                    href={project.appStore} target="_blank" rel="noopener noreferrer"
                    className="project-link project-link--appstore"
                    style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: COLORS.textMuted, fontWeight: 500, marginLeft: 'auto' }}
                >
            ...
            {project.link && !project.github && (
                <a
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: project.color, fontWeight: 600 }}
                >
                    <FiExternalLink size={14} /> Visit Live
                </a>
            )}
            <style>{`
                .project-link { transition: color 0.2s; }
                .project-link:hover { color: ${COLORS.textPrimary} !important; }
                .project-link--play:hover { color: #34d399 !important; }
                .project-link--appstore:hover { color: #60a5fa !important; }
            `}</style>
        </div>
    )
}
                ...
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                        <span style={{
                            ...
                            color: project.color, transition: 'background 0.3s, border-color 0.3s',
                        }}>
                            {project.icon}
                        </span>
                        <h3 style={{ fontSize: '1.05rem', fontWeight: 700, color: COLORS.textPrimary, margin: 0 }}>
                            {project.name}
                        </h3>
                    </div>
...
function FilterTab({ label, active, onClick }: FilterTabProps) {
    return (
        <button
            onClick={onClick}
            style={{
                padding: '8px 20px', borderRadius: '100px', fontFamily: 'inherit',
