...
import { cardSectionLabel, gradientText, gradientTextShort } from '../styles'
import type { StatItem } from '../types'
import type { CSSProperties } from 'react'

interface StatCardProps extends StatItem {
    /** Position in the grid — drives staggered entrance delay. */
    index: number
    /** Passed from the parent `useSectionInView` so we don't create a redundant observer. */
    inView: boolean
}
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                                    {STORE_LINKS.map(({ label, sub, icon, href, color, bg, border, hoverBg, hoverBorder }) => (
                                        <a
                                            ...
                                            style={{
                                                // Per-instance color tokens consumed by .store-link-btn:hover in index.css.
                                                // CSS custom properties require a string-indexed cast because
                                                // React's CSSProperties only covers standard properties.
                                                ['--store-bg']: bg,
                                                ['--store-bg-hover']: hoverBg,
                                                ['--store-border']: border,
                                                ['--store-border-hover']: hoverBorder,
                                                ['--store-shadow']: `${color}20`,
                                            } as CSSProperties}
                                        >
